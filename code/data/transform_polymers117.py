...
polymers_atomic_num_list = [6, 7, 8, 9, 14, 16, 0]  # 0 is for virtual node.
max_atoms = 87
n_bonds = 4


def one_hot_polymers117(data, out_size=87):
    num_max_id = len(polymers_atomic_num_list)
    assert data.shape[0] == out_size
    b = np.zeros((out_size, num_max_id), dtype=np.float32)
    for i in range(out_size):
        ind = polymers_atomic_num_list.index(data[i])
        b[i, ind] = 1.
    return b


def transform_fn_polymers117(data):
    node, adj, label = data
    # convert to one-hot vector
    # node = one_hot(node).astype(np.float32)
    node = one_hot_polymers117(node).astype(np.float32)
...
def get_val_ids():
    file_path = '../data/valid_idx_polymers.json'
