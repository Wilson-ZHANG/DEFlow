...
zinc50_atomic_num_list = [6, 7, 8, 9, 15, 16, 17, 35, 53, 0]  # 0 is for virtual node.
max_atoms = 38
n_bonds = 4


def one_hot_zinc50k(data, out_size=38):
    num_max_id = len(zinc50_atomic_num_list)
    assert data.shape[0] == out_size
    b = np.zeros((out_size, num_max_id), dtype=np.float32)
    for i in range(out_size):
        ind = zinc50_atomic_num_list.index(data[i])
        b[i, ind] = 1.
    return b


def transform_fn_zinc50k(data):
    node, adj, label = data
    # convert to one-hot vector
    # node = one_hot(node).astype(np.float32)
    node = one_hot_zinc50k(node).astype(np.float32)
...
def get_val_ids():
    file_path = '../data/valid_idx_zinc50k.json'
