...
def parse():
    ...
    parser.add_argument('--data_name', type=str, default='qm9',
                        choices=['qm9', 'zinc250k', 'moses', 'moses17k','moses1k','polymers117','nucle'],
...
if data_name == 'qm9':
    max_atoms = 9
elif data_name == 'nucle':
    max_atoms = 40
elif data_name == 'zinc250k':
    max_atoms = 38
elif data_name == 'moses':
    max_atoms = 38
elif data_name == 'moses17k':
    max_atoms = 38
elif data_name == 'moses1k':
    max_atoms = 38
elif data_name == 'nucle':
    max_atoms = 40
elif data_name == 'polymers117':
    max_atoms = 87
else:
...
if data_name == 'qm9':
    ...
    print('Preprocessing Nucleophilicity data:')
    df_nucle = pd.read_csv('nucle.csv', index_col=0)
    labels = ['homo', 'lumo']
    parser = DataFrameParser(preprocessor, labels=labels, smiles_col='smiles')
    result = parser.parse(df_nucle, return_smiles=True)
    dataset = result['dataset']
    smiles = result['smiles']
elif data_name == 'polymers117':
    print('Preprocessing Polymers117 data:')
    df_polymers = pd.read_csv('polymers117.csv', index_col=0)
    labels = []
    parser = DataFrameParser(preprocessor, labels=labels, smiles_col='smiles')
    result = parser.parse(df_polymers, return_smiles=True)
    dataset = result['dataset']
    smiles = result['smiles']
elif data_name == 'zinc250k':
    ...
    print('Preprocessing Moses data')
    # dataset = datasets.get_zinc250k(preprocessor)
    df_moses = pd.read_csv('moses-test.csv')
    print(df_moses)# Caution: Not reasonable but used in used in chain_chemistry\datasets\zinc.py:
    # 'smiles' column contains '\n', need to remove it.
    # Here we do not remove \n, because it represents atom N with single bond
    labels = []
    parser = DataFrameParser(preprocessor, labels=labels, smiles_col='smiles')
    ...
    print('Preprocessing Moses data')
    # dataset = datasets.get_zinc250k(preprocessor)
    df_moses = pd.read_csv('moses17k.csv')
    print(df_moses)# Caution: Not reasonable but used in used in chain_chemistry\datasets\zinc.py:
    # 'smiles' column contains '\n', need to remove it.
    # Here we do not remove \n, because it represents atom N with single bond
    labels = []
    ...
    print('Preprocessing Moses data')
    # dataset = datasets.get_zinc250k(preprocessor)
    df_moses = pd.read_csv('moses1k.csv')
    print(df_moses)# Caution: Not reasonable but used in used in chain_chemistry\datasets\zinc.py:
    # 'smiles' column contains '\n', need to remove it.
    # Here we do not remove \n, because it represents atom N with single bond
    labels = []
