...
from mflow.models.acon import AconC,MetaAconC
...
class Flow(nn.Module):
    ...
    def forward(self, input):
        ...
        if det2 is not None:
            logdet = logdet + det2
        '''
        #E0 = self.energy_model.energy(out)
        E0 = out
        E = E0

        for i in range(1):
            # proposal step
            dx = 0.1 * torch.zeros_like(out).normal_()
            xprop = out + dx
            #Eprop = self.energy_model.energy(xprop)
            Eprop = xprop

            # acceptance step
            acc = (torch.rand(out.shape[0], 1) < torch.exp(-(Eprop - E))).float()  # selection variable: 0 or 1.
            out = (1-acc) * out + acc * xprop
            E = (1-acc) * E + acc * Eprop
