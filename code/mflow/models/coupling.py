...
from mflow.models.acon import MetaAconC, AconC
import math
...
class AffineCoupling(nn.Module):  # delete
    ...
    def forward(self, input):
        ...
        if self.affine:
            ...
            out_b = in_b * torch.exp(s) + t * (1.1 + torch.cos(s))   #  different affine bias , no difference to the log-det # (2,6,32,32) More stable, less error
            #out_b = (in_b+t)*s
    ...
    def reverse(self, output):
        ...
        if self.affine:
            ...
            in_b = (out_b - t * (1.1 + torch.cos(-s))) * torch.exp(-s)  # More stable, less error   s must not equal to 0!!!
            #in_b = out_b / s - t
        else:
...
class GraphAffineCoupling(nn.Module):
    ...
    def forward(self, adj, input):
        ...
        if self.affine:
           # out = masked_x + (1 - self.mask) * (input * torch.exp(s) + t * (1.1 + torch.cos(s)))
    ...
    def reverse(self, adj, output):
        ...
        if self.affine:
             #input = masked_y + (1 - self.mask) * (output - t * (1.1 + torch.cos(-s)) * torch.exp(-s))
